...
async function findCoinId(coinQuery: string): Promise<string | null> {
  ...
  const exactNameMatch = coins.find(coin => coin.name.toLowerCase() === query);
  if (exactNameMatch) return exactNameMatch.id;

  const symbolMatches = coins.filter(coin => coin.symbol.toLowerCase() === query);
  if (symbolMatches.length === 1) return symbolMatches[0].id;
  if (symbolMatches.length > 1) return symbolMatches[0].id;

  for (const coin of coins) {
    if (Object.values(coin.platforms).some(address => address.toLowerCase() === query)) {
      return coin.id;
    }
  }
...
export const getDecision: Action = {
  name: "GET_DECISION",
  description: "Determine whether to BUY, HOLD, or AVOID a cryptocurrency based on market metrics.",
  ...
  handler: async (
    ...
    let coinQuery = String(options.coinQuery || message?.content?.coinName || state?.coinName || "").trim();
    if (!coinQuery && message?.content?.text) {
      coinQuery = message.content.text.match(/buy\s+([a-zA-Z0-9]+)/i)?.[1] || "";
    }

    if (!coinQuery) {
      if (callback) callback({ text: "Can't recognize the coin name, ID, or contract address. Try again?" });
      return true;
    }
    ...
    const trendingData = (await getTrending.handler(runtime, message, state, {})) as { content?: { trending?: { coins?: { id: string; score: number }[] } } };
    const priceData = (await getPrice.handler(runtime, message, state, {})) as { content?: { price?: number; marketCap?: number; volume24h?: number } };
    const topGainersLosers = (await getTopGainersLosers.handler(runtime, message, state, {})) as { content?: { volatility?: number; priceChange24h?: number } };

    const trendingCoin = trendingData?.content?.trending?.coins?.find(c => c.id === coinId);
    ...
    const marketCapScore = marketCap > 10_000_000_000 ? 100 : (marketCap / 10_000_000_000) * 100;

    const volume24h = priceData?.content?.volume24h ?? 0;
    const liquidityScore = volume24h > 100_000_000 ? 100 : (volume24h / 100_000_000) * 100;

    const totalScore = (trendingScore * 0.3) + (stabilityScore * 0.3) + (marketCapScore * 0.2) + (liquidityScore * 0.2);
